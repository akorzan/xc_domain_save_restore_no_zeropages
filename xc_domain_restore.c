...
#define LOG_FILE_PATH "/var/log/GW_xc_restore.log"   /* File path for custom logging */
...
{
    /* Count not only holds the batch size, it can also hold special cases! */
    int count, countpages, oldcount, i;
    void* ptmp;
    /* GW
     * This array keeps track of the zero pages. During the batch loop it
     * will be passed and every loop will check if the current index is a zero
     * page.  If it is, it will populate 4kbytes with 0s and continue to the next
     * iteration. */
    unsigned char zeroPages[MAX_BATCH_SIZE];
    /* unsigned int run, ran; Used for eariler attempt */
    ...
    default:
        if ( (count > MAX_BATCH_SIZE) || (count < 0) ) {
            ...
            /* Read array of zeroPages */
            if ( RDEXACT(fd, &zeroPages, sizeof(zeroPages)) ) {
                PERROR("Error when reading the list of this batch's zero pages.");
                return -1;
            }

#if DEBUG_ALT_LOG == 1
            custom_printf(LOG_FILE_PATH, "Batch Size: %u  Boolean Array of Zero Pages", count);
            {
                int loopCounter;
                FILE *logFile;
                logFile = fopen(LOG_FILE_PATH, "a+");
                for(loopCounter = 0; loopCounter < count; loopCounter++) {
                    fprintf(logFile, "%x", zeroPages[loopCounter]);
                }
                fprintf(logFile, "\n");
                fclose(logFile);
            }
...
#if DEBUG_ALT_LOG == 1
    custom_printf(LOG_FILE_PATH, "Actual count of pages to allocate: %d", countpages);
#endif
    for(i = 0; i < count; i++) {
        unsigned long pagetype = buf->pfn_types[i] & XEN_DOMCTL_PFINFO_LTAB_MASK;
        /* Skip pages that aren't present */
        if (pagetype == XEN_DOMCTL_PFINFO_XTAB) {

#if DEBUG_ALT_LOG == 1
            custom_printf(LOG_FILE_PATH, "Skipped");
#endif
            continue;
        }
        
        if (pagetype == 0 && zeroPages[i]) {
#if DEBUG_ALT_LOG == 1
            custom_printf(LOG_FILE_PATH, "Attempt to memset at index %d oldcount %d", i, oldcount);
#endif
            memset(buf->pages + oldcount * PAGE_SIZE, 0, PAGE_SIZE);
        } else {

#if DEBUG_ALT_LOG == 1
            custom_printf(LOG_FILE_PATH, "Attempt to read at index %d oldcount %d", i, oldcount);
#endif
            if ( RDEXACT(fd, buf->pages + oldcount * PAGE_SIZE, PAGE_SIZE) ) {
                PERROR("Error when reading page");
                return -1;
            }
        }
        oldcount++;
...
#if DEBUG_ALT_LOG == 1
    custom_printf(LOG_FILE_PATH, "Finished batch");
#endif

    /* Here is my earlier attempt at reading in with runs */
#if 0
    /* Here we actually read in the pages of the batch.  Here I will check
     * when to insert zero pages. */
    for(run = ran = i = 0; i < MAX_BATCH_SIZE; i++) {
        unsigned long pagetype = buf->pfn_types[i] & XEN_DOMCTL_PFINFO_LTAB_MASK;
        /* Skip pages that aren't present */
        if (pagetype == XEN_DOMCTL_PFINFO_XTAB) {
            continue;
        ...
        /* If the page is normal and a zero page, then read in non-zero pages
         * that have accumulated before it. */
        if (pagetype == 0 && zeroPages[i]) {
            void* startWritingPageHere = buf->pages + (oldcount + ran) * PAGE_SIZE;
            /* If there are accumlated zero pages, read them */
            if (!run) {
                if ( RDEXACT(fd, startWritingPageHere, (run) * PAGE_SIZE) ) {
                    PERROR("Error when reading pages");
                    return -1;
                }
            }
            /* This counter serves to keep track of how many pages we have iterated
             * through that were not skipped.  Since we never recieve the maximum batch
             * in pages, then this is neccesary. */
            ran = ran + run;
            /* Now manually set memory to 0 for the next 4kb page */
            startWritingPageHere = buf->pages + (oldcount + ran) * PAGE_SIZE;
            memset(startWritingPageHere, 0, PAGE_SIZE);
            ran = ran + 1;
            run = 0;
        } else {
            /* We have either a rewritten pagetable page or a non-zero normal
             * page: accumulate it for reading. */
            run++;
        }
    }
#endif
