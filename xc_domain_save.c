 ...
 * Declerations.
 */
#define DEBUG_DUMP_MEMORY 0   /* BROKEN  If set to 1, program will be compiled to dump memory during each live loop to the following log file.
                                 Currently, the program will only dump memory that is hashed to 0 using csum_page(void* page). */
#define PAGE_LOG_FILE_PATH "/var/log/GW_xen_pages.log"   /* File path for dumping each memory page */

#define DEBUG_ALT_LOG 0   /* If set to 1, program will be compiled to use in addition to normal logging, an alternate log */
#define LOG_FILE_PATH "/var/log/GW_xen.log"   /* File path for custom logging */

/* These declerations are for logging. */
#if DEBUG_DUMP_MEMORY == 1 || DEBUG_ALT_LOG == 1
static char* getMonth(int n);
static int custom_printf(char* filePath, char* str, ...);
#endif
...
#if DEBUG_DUMP_MEMORY == 1 || DEBUG_ALT_LOG == 1
/** GW
 * @param n the int to be passed represent the month
 * @return char* the String form of the month
 */
static char* getMonth(int n) {
    switch (n) {
    case 0:
...
/** GW
 * This method provides a way to print custom Log messages provided a file path.
 * @param path The char* of the path to the Log.
 * @param str The char* that is appended to the Log.
 * @return int the Error Code where 0 represents success and -1 failure.s
 */
static int custom_printf(char* filePath, char* str, ...) {
    va_list arguments;
    static struct tm *Sys_T = NULL;
    ...
    if(logFile) {
        fprintf(logFile, "%s %2d %02d:%02d:%02d ", (char*) getMonth(Sys_T->tm_mon),
                Sys_T->tm_mday, Sys_T->tm_hour, Sys_T->tm_min, Sys_T->tm_sec);
        vfprintf (logFile, str, arguments);
        fprintf(logFile, "\n");
        fclose(logFile);
        return 0;
    } else {
...
/** GW
 * This function actually performs the migration:
 * xc_interface *xch === ???
 * int io_fd == IO descriptor for either a file on disk to write to, or a network socket to migrate to
 * uint32_t dom == virtual machine ID number to move
 * uint32_t max_iters == maximum number of iterations to cycle through
 * in each iteration, copy VM's memory and track which pages get dirtied, next iteration just send the dirtied pages
 */
...
{
    /* GW
     * Some information tracking variables */
    static int numberOfZeroPages;

    /* For every iteration of a batch, this array of booleans
     * will keep track of zero pages. */
    unsigned char zeroPages[MAX_BATCH_SIZE];
    
    xc_dominfo_t info;
    DECLARE_DOMCTL;

    int rc = 1, frc, i, j, k, last_iter = 0, iter = 0;
...
#if DEBUG_ALT_LOG == 1
    custom_printf(LOG_FILE_PATH, "Migration Started");
#endif
...
#define ratewrite(fd, live, buf, len) ratewrite_buffer(xch, last_iter, &ob, (fd), (live), (buf), (len))
    ...
    while(1) {
        ...
        /* GW */
        if (iter)
            custom_printf(LOG_FILE_PATH, "\"Zero Pages\" sent last loop: %d", numberOfZeroPages);
        numberOfZeroPages = 0;
        if (iter)
            custom_printf(LOG_FILE_PATH, "Saving memory: Finished Loop %d out of %d possible (Sent %u and skipped %u memory pages)",
                          iter, DEF_MAX_ITERS, sent_this_iter, skip_this_iter);
#endif
        ...
        while ( N < dinfo->p2m_size )
            {
                ...
                for  ( batch = 0;
                       ...
                       N++ )
                    {
                        int n = N;
# if 0
                        custom_printf(PAGE_LOG_FILE_PATH, "%d pfn= %08lx mfn= %08lx %d",
                                      iter, (unsigned long)n,
                                      hvm ? 0 : pfn_to_mfn(n),
                                      test_bit(n, to_send));
                        if ( !hvm && is_mapped(pfn_to_mfn(n)) )
                            custom_printf(PAGE_LOG_FILE_PATH, "  [mfn]= %08lx",
...
# endif
                        ...
                        if ( completed )
                        ...
                        else
                            {
                                ...
                                */
                                ...
                                /* GW
                                ** So here I am going to skip adding this page to the batch if it is a zero page
                                */
#if 0
                                if (hvm) {
                                    custom_printf(LOG_FILE_PATH, "HVM Guest, n: %d", n);
                                } else
                                    custom_printf(LOG_FILE_PATH, "PV Guest, n: %d, mfn: %x", pfn_to_mfn(n));
#endif
                ...
                /* GW
                   Here we have finally populated pfn_type with page file types.
                   Region_base now points to a fresh consitent array of memory
                   that is to be sent to the recieving end.
                */
                
#if DEBUG_ALT_LOG == 1
                custom_printf(LOG_FILE_PATH, "Started a batch of %d pages.", batch);
#endif
                /* GW
                 * It seems like this loop is used to check each page -- for exactly
                 * what I am unclear.  This is where I wiill implement zero page checking.
                 * Run is used in this loop to count amount of pages in the batch.
                 * If at the end of this loop the run is zero, then the batch will
                 * be skipped.
                 */
                for ( run = j = 0; j < batch; j++ ) {
                    ...
                    /* This is where the zeroPages array gets populated with booleans.
                     * If the batch is bigger than the MAX_BATCH_SIZE (1024), then this
                     * run must be the last run.  So let's not optimize it for now. */
                    if (batch <= MAX_BATCH_SIZE) {
                        int checksum;
                        /* Each batch starts at region_base.  Thus, the offset of                        
                         * the counter, j, can dictate it page withing a batch of pages. */
                        checksum = csum_page(region_base + (PAGE_SIZE*j));
                        if (!checksum) {
                            zeroPages[j] = 1;
                            numberOfZeroPages++; /* Stat keeping */
                        } else {
                            zeroPages[j] = 0;
                        }
#if DEBUG_DUMP_MEMORY == 1
                        { /* Dump Memory (what we suspect are zero pages) to a file. */
                            unsigned int *p = (void*) region_base + (PAGE_SIZE*j);
                            int i;
                            static FILE *logFile;
    
                            custom_printf(PAGE_LOG_FILE_PATH, "Address: 0x%x", (region_base + (PAGE_SIZE*j)));
                            custom_printf(PAGE_LOG_FILE_PATH, "Does pfn_type == XEN_DOMCTL_PFINFO_XTAB? : %d", (pfn_type[j] == XEN_DOMCTL_PFINFO_XTAB));
                            logFile = fopen(PAGE_LOG_FILE_PATH, "a+");
    
                            for ( i = 0; i < (PAGE_SIZE/sizeof(unsigned int)); i++ ) {
                                fprintf(logFile, "%x", p[i]);
                            }
                ...
                /* This must be where we send out the size of the batch to the recieving end */
                if ( wrexact(io_fd, &batch, sizeof(unsigned int)) ) {
                    PERROR("Error when writing to state file (2)");
                    goto out;
                }
                /* So this is where I will send out the array of zero pages in this batch */
                if ( wrexact(io_fd, &zeroPages, sizeof(zeroPages)) ) {
                    PERROR("Error when writing the list of zero pages in this bactch.");
                    goto out;
                }
                ...
                for ( j = 0; j < batch; j++ )
                    {
                        ...
                        if ( pagetype != 0 ) {
                            ...
                            if ( run ) {
#if DEBUG_ALT_LOG == 1
                                custom_printf(LOG_FILE_PATH, "Actual count of pages to send: %d", run);
#endif
                                for(k = 0; k < run; k++) {        
                                    if (zeroPages[j-run+k]) {
#if DEBUG_ALT_LOG == 1
                                        custom_printf(LOG_FILE_PATH, "Skipping zero page at index %d", k);
#endif
                                    } else {

#if DEBUG_ALT_LOG == 1
                                        custom_printf(LOG_FILE_PATH, "Attempt to write at index %d", k);
#endif
                                        if ( ratewrite(io_fd, live, 
                                                       (char*)region_base+(PAGE_SIZE*(j-run+k)), 
                                                       PAGE_SIZE) != PAGE_SIZE ) {
                                ...
                                /* Earlier attempt at sending pages without zeropages */ 
# if 0
                                unsigned int runInThisLoop, ranInThisLoop;
                                /* Write out all the pages that are nonzero */
                                for (ranInThisLoop = runInThisLoop = k = 0; k < run; k++) {
                                    if (zeroPages[j - run + k]) {
                                        /* write out any pages that have accumlated
                                         * within this loop */
                                        void* startReadingPageHere = region_base + (j - run + ranInThisLoop) * PAGE_SIZE;
                                        
                                        if (ratewrite(io_fd, live, 
                                                      (char*) startReadingPageHere, 
                                                      PAGE_SIZE * (runInThisLoop)) != PAGE_SIZE * (runInThisLoop)) {
                                            PERROR("Error when writing to state file (4a)"
                                                   " (errno %d)", errno);
                                            goto out;
                                        }
                                        
                                        /* Basically we are skipping the current page to write out */
                                        ranInThisLoop += runInThisLoop + 1;
                                        runInThisLoop = 0;
                                    } else {
                                        runInThisLoop++;
                                    }
...
# endif                        
...
#if DEBUG_ALT_LOG == 1
                            custom_printf(LOG_FILE_PATH, "Skip at j:%d", j);
#endif
                            continue;
                        }
    ...
    custom_printf(LOG_FILE_PATH, "Success!");
#endif
...
#if DEBUG_ALT_LOG == 1
    custom_printf(LOG_FILE_PATH, "Ending Migration.");
#endif
    return !!rc;
}
